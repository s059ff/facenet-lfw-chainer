...
class FaceNet(chainer.Chain):
    ...
    def __init__(self):
        ...
        with self.init_scope():
            self.conv1 = L.Convolution2D(None, 64, ksize=(3, 3), pad=1, stride=1)
            self.bn1 = L.BatchNormalization(64)
            self.conv2 = L.Convolution2D(None, 128, ksize=(3, 3), pad=1, stride=1)
            self.bn2 = L.BatchNormalization(128)
            self.conv3 = L.Convolution2D(None, 256, ksize=(3, 3), pad=1, stride=1)
            self.bn3 = L.BatchNormalization(256)
            self.conv4 = L.Convolution2D(None, 512, ksize=(3, 3), pad=1, stride=1)
    ...
    def __call__(self, h):
        ...
        # Enforce to place on hypersphere (|h| == 1)
        h = F.normalize(h)
        return h
